...
// 1. Generate static routes for every chapter in the database
export async function generateStaticParams() {
  try {
    const res = await fetch(`${STRAPI_URL}/api/chapters`);
    ...
    console.error("Failed to fetch chapters for static params:", error);
    return [];
  }
}

export default async function ExercisesPage({ params }) {
  // Await the params to get the chapterId from the URL
  const resolvedParams = await params;
  const chapterId = resolvedParams.chapterId;

  // 2. Fetch Exercises linked specifically to this chapter
  let exercises = [];
  try {
    ...
    <div className="flex flex-col min-h-screen max-w-7xl w-full bg-card rounded mb-8 mx-auto">
      ...
      {exercises.length === 0 ? (
        <div className="p-8 mt-4 text-center bg-gray-100 dark:bg-zinc-800 rounded-lg mx-4">
          <p className="text-gray-600 dark:text-gray-300">No exercises found for this chapter yet.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-4 gap-4 mx-4 mt-6">
          {exercises.map((exercise) => {
            const exerciseTitle = exercise?.attributes?.Title || exercise?.Title || "Untitled Exercise";
              ...
              >
                ...
                <div className="mt-auto pt-2">
                  ...
                  <Button className="w-full text-center text-white transition-colors cursor-pointer">
                    {/* Routes to the final study room where the questions will live */}
                      Questions
                  </Button>
                    </Link>
